...
import { ScanFace, PenLine, CheckCircle2, RotateCcw, Upload, Pencil } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/movil/asistencia")({
    ...
    <MobileFrame title="Registro de asistencia">
      <Card className="space-y-3 p-4">
        <div className="space-y-1.5">
          ...
          <Input defaultValue={profile?.nombre ?? ""} className="bg-card" />
        </div>
        <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          ...
          <Input readOnly value={`${acta.fecha} · 09:41`} className="bg-secondary/50" />
        </div>
      </Card>

      <Card className="p-4">
        <p className="mb-2 text-sm font-medium">Reconocimiento facial (simulado)</p>
        <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-accent/50 bg-accent/5 py-6">
          <ScanFace className={facial === "scanning" ? "size-12 animate-pulse text-accent-foreground" : facial === "ok" ? "size-12 text-success" : "size-12 text-muted-foreground"} />
          ...
          {facial !== "ok" && (
            <Button size="sm" variant="outline" className="mt-3" onClick={escanear} disabled={facial === "scanning"}>
      ...
      <Button
        variant="hero"
        className="w-full"
        disabled={facial !== "ok" || !firma}
        onClick={() => toast.success("Asistencia confirmada", { description: "Tu asistencia fue registrada correctamente." })}
      >
